...
import { HostRejectedstatus } from '../../api/Services/HostgetData';
import loadgifs from '../../../src/Asset/loadingadmin.gif';
import Modals from '../../../src/components/Re-components/Modal';
import { useSelector } from 'react-redux';

function HostDeniedlist({status}) {
  ...
  useEffect( ()=>{
    ...
    const fetchUser = async () =>{
       try{
          const response = await HostRejectedstatus(token);
 ...
 if (isLoading) {
  return <div className='w-full'>
     <div className='flex justify-center items-center w-full'>
        <img className='w-48 h-48 justify-center bg-bg-gradient-to-r from-regal-blue via-regal-blue1 to-regal-blue2' src={loadgifs} alt='loading.....'></img>
     </div>
  </div>;
}
            ...
            <div className='w-full border border-black rounded-md mt-5 '>
              ...
              <div className='flex mx-14 justify-center mt-7 mb-14'>
                 <div className='flex flex-row'>
                  ...
                  <h1 className='text-xl font-bold text-red-500 mt-[-1px] mx-2'>Rejected</h1>
                  </div>
              </div>
            </div>
